...
function cartorder_detail(a_ob, popup, buy_now) {
	var num = $('#pnum').val();
	var id_price = $('.content_item_products .active_product').attr('id_price');
	...
	$.ajax({
		type : "POST",
		url : nv_base_siteurl + 'index.php?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=setcart' + '&id=' + id + '&id_price=' + id_price +  "&group=" + group + "&nocache=" + new Date().getTime(),
		data : 'num=' + num,
		success : function(data) {
			var s = data.split('_');
			var strText = s[1];
...
function payment_vitien( order_id, checkss, lang_confirm )
{
	if (confirm(lang_confirm)) {
		$.ajax({
			type : "GET",
			url : nv_base_siteurl + 'index.php?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=point' + '&payvitien=1&checkss='+checkss+'&order_id=' + order_id + "&nocache=" + new Date().getTime(),
			data : '',
			success : function(data) {
				var s = data.split('_');
				if(data == '1')
				{
					window.location.href = nv_base_siteurl + 'index.php?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=users';
				}
				else if(data == '0')
				{
					alert( 'Bạn đã thanh toán trước đó!' );
				}
				else
				{
					if( s[0] == 'OK' )
					{
						$.ajax({
								type : "POST",
								url : nv_base_siteurl + 'index.php?' + nv_lang_variable + '=' + nv_lang_data + '&' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=active_pay' + '&order_id=' + order_id + "&action=pay",
								data : 'save=1',
								success : function(data) {
									//alert( data );
									window.location.href = window.location.href;
